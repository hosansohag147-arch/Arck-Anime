...
export default function App() {
  ...
  const [selectedDomain, setSelectedDomain] = useState<string>("watchanimeworld.net");
    ...
    const fetchStreamSettings = async () => {
       ...
       try {
         ...
         if (response.ok) {
           ...
           // Keep logs in sync with watchanimeworld.net active crawl alerts
           fetchBotStatus();
         ...
         const suffix = currentDub === "DUB" ? "-dub" : "";
 setIframeUrl(`[https://${activeDomain}/episode/${safeSlug}-1x${currentEpisode}/#options-0`](https://${activeDomain}/episode/${safeSlug}-1x${currentEpisode}/#options-0`));
         setPlayerMode('iframe');
       } finally {
           ...
           {isFetchingStream ? (
              ...
              />
               ...
               >
                 ...
                 {/* Invisible overlay — blocks touch drag on iframe */}
                 <div style={{
                   position: 'absolute',
                   inset: 0,
                   zIndex: 10,
