...
export default function App() {
  ...
  // Iframe dynamic offset state
  const [iframeOffset, setIframeOffset] = useState<number>(0);
               ...
               <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: '#000' }}>
                 <iframe
                   ...
                   style={{
                     ...
                     left: '0',
                     width: '100%',
                     height: iframeOffset < 0 ? `calc(100% + ${Math.abs(iframeOffset) * 2}px)` : `calc(100% + ${iframeOffset * 2}px)`,
                     pointerEvents: 'auto',
                     background: '#000',
                     transition: 'top 0.3s ease',
                   }}
                   onLoad={() => {
                     // Try to detect video element position inside iframe
                     try {
                       const iframe = iframeRef.current;
                       if (!iframe) return;
                       const iframeDoc = iframe.contentDocument || iframe.contentWindow?.document;
                       if (!iframeDoc) {
                         // Cross-origin: fallback to safe default
                         setIframeOffset(-340);
                         return;
                       }
                       const video = iframeDoc.querySelector('video') ||
                                     iframeDoc.querySelector('.jw-video') ||
                                     iframeDoc.querySelector('iframe') ||
                                     iframeDoc.querySelector('.plyr__video-wrapper') ||
                                     iframeDoc.querySelector('#player');
                       if (video) {
                         const rect = (video as HTMLElement).getBoundingClientRect();
                         const containerH = iframe.clientHeight;
                         const videoTop = rect.top;
                         const offset = -(videoTop - (containerH - rect.height) / 2);
                         setIframeOffset(Math.max(-600, Math.min(0, offset)));
                       } else {
                         setIframeOffset(-340);
                       }
                     } catch {
                       // Cross-origin fallback: center by assuming standard layout
                       setIframeOffset(-340);
                     }
                   }}
                   allowFullScreen
