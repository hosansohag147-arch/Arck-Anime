...
export default function App() {
  ...
  // Iframe ref for dimension calculation
        ...
        <div id="video-player-container" className="w-full aspect-video bg-black rounded-3xl overflow-hidden ring-1 ring-white/10 shadow-[0_30px_60px_rgba(229,9,20,0.2)] flex items-center justify-center mb-6 relative border border-white/5">
           {isFetchingStream ? (
              ...
              />
               ...
               /*
                * ToonStream iframe crop strategy:
                * ToonStream page layout (approximate, desktop scaled to mobile):
                *   ~0px    - page top / navbar
                *   ~380px  - info card starts  (title, tags, description, rating)
                *   ~780px  - VIDEO PLAYER starts  <-- we want this at top=0
                *   ~1220px - video player ends
                *   ~1220px - more content below
                *
                * We render the iframe much taller than the container and shift it up
                * so only the video band is visible inside the clipped container.
                *
                * The iframe is rendered at a SCALED width wider than 100% so that
                * ToonStream's desktop layout fits and the video fills the container.
                *
                * Tuning knobs (px values assume iframe intrinsic width ~1280px
                * scaled into our container):
                *   VIDEO_TOP_OFFSET  – how many px from iframe top to video top
                *   IFRAME_SCALE      – scale factor to zoom iframe content
                */
               (() => {
                 const containerW = iframeRef.current?.parentElement?.clientWidth || 390;
                 // ToonStream renders a ~1280px wide desktop layout inside the iframe.
                 // We scale it so the video fills our container width.
                 const IFRAME_INTRINSIC_W = 1280;
                 const scale = containerW / IFRAME_INTRINSIC_W;

                 // After scaling, the video top sits roughly 780px * scale from the iframe top.
                 // We shift the iframe up by that much so video aligns with container top.
                 const VIDEO_TOP_PX = 780; // px in iframe's own coordinate space
                 const shiftUp = VIDEO_TOP_PX * scale;

                 // Video height in iframe coords ~440px, scaled:
                 const VIDEO_HEIGHT_PX = 440;
                 const scaledVideoH = VIDEO_HEIGHT_PX * scale;

                 // Container height from aspect-video class (~containerW * 9/16)
                 const containerH = containerW * (9 / 16);

                 // Extra vertical space to render (above shift + below video)
                 const iframeRenderH = shiftUp + containerH + 200;
                   ...
                   >
                     <iframe
                       ...
                       style={{
                         ...
                         top: `-${shiftUp}px`,
                         left: '0',
                         width: `${IFRAME_INTRINSIC_W}px`,
                         height: `${iframeRenderH}px`,
